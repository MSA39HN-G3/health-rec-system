...
> **Phiên bản: 1.4.3** — Ghi nhận thêm: khi PATCH `avatar_object_key`, BE tự xóa
> object cũ trên R2 (FE không cần gọi API delete). Tham chiếu chi tiết trong
> [`FE_DEPARTMENT_AVATAR_UPLOAD.md`](./FE_DEPARTMENT_AVATAR_UPLOAD.md) mục
> "Dọn dẹp avatar cũ". Phiên bản 1.4.2 (thêm trường `avatar_object_key`) và
> 1.4.1/1.4.0 (`is_active`, toggle UI read-only) giữ nguyên.
...
```jsonc
{
  ...
  "is_active": true                       // tùy chọn — mặc định false. Nếu true mà không có head_doctor_id hợp lệ -> 400.
}
```

> **Tối thiểu chỉ cần `name`.** Các trường `location`, `description`, `keywords`, `conditions`,
> `ai_metadata`, `head_doctor_id`, `is_active` đều **tùy chọn**.
>
> ⚠️ **Không gửi `code`** (hệ thống tự sinh `CK-NNN`).
>
> 🆕 **`is_active` (mặc định `false`):** cờ bật/tắt khoa ngay khi tạo. Quy tắc:
> - `is_active = false` (mặc định) → tạo khoa ở trạng thái tạm dừng, không cần trưởng khoa.
> - `is_active = true` → **bắt buộc** `head_doctor_id` phải trỏ vào một user tồn tại
>   và đang có role `doctor`. Nếu thiếu hoặc không hợp lệ, BE trả `400` với message
>   "Phải chọn trưởng khoa khi bật hoạt động khoa."
...
- **Tương tác với `is_active`:** muốn tạo khoa ở trạng thái hoạt động ngay thì **bắt buộc**
  chọn `head_doctor_id` hợp lệ; nếu không, hệ thống sẽ từ chối với `400`
  ("Phải chọn trưởng khoa khi bật hoạt động khoa.").
- **Quyết định `is_active`:** giá trị lưu xuống DB đúng bằng giá trị client gửi (`false` mặc
  định). Hệ thống chỉ áp ràng buộc: nếu `is_active = true` mà **không có** `head_doctor_id`
  hợp lệ thì trả `400`.
...
| **400** | `is_active = true` nhưng **không có** `head_doctor_id` hợp lệ | "Phải chọn trưởng khoa khi bật hoạt động khoa." |
...
- **`is_active` trong body tạo khoa (mục 3):** mặc định `false`. Bật (`true`) chỉ hợp lệ khi
  đã chọn được `head_doctor_id` là bác sĩ; nếu không BE trả `400`.
- **Toggle enable/disable trên UI chỉ là hiển thị — KHÔNG gửi request lên BE.** Vì `is_active` do BE tự suy ra từ `head_doctor_id`, mọi giá trị FE cố gửi sẽ bị BE bỏ qua. Cách duy nhất để chuyển trạng thái khoa là **bổ nhiệm / gỡ trưởng khoa** qua `PATCH /api/v1/departments/{id}` với `head_doctor_id` (xem mục [6](#6-cập-nhật-khoa--patch-apiv1departmentsid)). FE hiển thị toggle dưới dạng **read-only switch / badge** (xanh = active, xám = inactive) để người dùng hiểu trạng thái mà không tạo kỳ vọng có thể bấm để đổi.
...
- [ ] Form tạo khoa: chỉ `name` bắt buộc; **không có ô `code`**; `keywords`/`conditions` dùng tag-input (mảng chuỗi); có thể có ô toggle `is_active` (mặc định tắt) và **bắt buộc chọn trưởng khoa khi bật** (BE sẽ trả `400` nếu thiếu).
...
| **1.4.1** | Thêm trường `is_active` (bool, mặc định `false`) vào body `POST /departments`. Bật `true` bắt buộc có `head_doctor_id` hợp lệ, nếu không BE trả `400` ("Phải chọn trưởng khoa khi bật hoạt động khoa."). |
| **1.4.0** | Bổ sung nút toggle enable/disable trên UI ở dạng **read-only** (chỉ phản ánh trạng thái), không gửi request lên BE. |
